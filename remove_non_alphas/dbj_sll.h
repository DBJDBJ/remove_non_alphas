...
#ifndef DBJ_UNUSED
#ifdef __cplusplus
#define DBJ_UNUSED(x) [[maybe_unused]] x
#else
...
#define DBJ_SLL_HEAD_KEY ((unsigned long)-1)
	...
	/********************************************************/
	inline dbj_sll_node * dbj_sll_node_new()
	{
		dbj_sll_node * new_
	...
	/********************************************************/
	inline bool is_dbj_sll_head(dbj_sll_node * head_) {
		assert(head_);
		if (head_) {
	...
	inline bool is_dbj_sll_tls_empty() { return (dbj_sll_tls_head()->next == 0); }
	inline bool is_dbj_sll_empty(dbj_sll_node * head_) { return head_->next == 0; }
	inline dbj_sll_node * dbj_sll_next(dbj_sll_node * current_)
	{
		assert(current_);
		return current_->next;
	}
	inline const char * dbj_sll_data(dbj_sll_node * current_)
	{
		assert(current_);
		return current_->data;
	}
	inline const unsigned long dbj_sll_key(dbj_sll_node * current_)
	{
		assert(current_);
		return current_->key;
	}
	/********************************************************/
	inline dbj_sll_node * dbj_sll_tail(dbj_sll_node * head_)
	{
		/* get the head */
	...
	inline void dbj_sll_erase(dbj_sll_node * head_)
	{
	...
	inline void dbj_sll_erase_with_head(dbj_sll_node * head_)
	{
		dbj_sll_erase(head_);
	...
	*/
	inline dbj_sll_node * dbj_sll_find(dbj_sll_node * head_, unsigned long k_)
	{
	...
	*/
	inline dbj_sll_node * dbj_sll_foreach
	...
	/********************************************************/
	inline size_t dbj_sll_count(dbj_sll_node * head_)
	{
		size_t count_ = 0;
	...
	*/
	...
	/* sll visitors                                         */
	/********************************************************/
	inline bool dbj_sll_node_dump_visitor(dbj_sll_node * node_) {
	...
	*/
	inline bool dbj_sll_remove_tail_visitor(dbj_sll_node * node_)
	{
	...
	inline dbj_sll_node * dbj_sll_remove_tail(dbj_sll_node * head_)
	{
